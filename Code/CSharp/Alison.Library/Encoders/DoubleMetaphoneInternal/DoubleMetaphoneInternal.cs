...
namespace Alison.Library.Encoders
{
	...
	/// </summary>
	internal class DoubleMetaphoneInternal
	{
		public const int METAPHONE_KEY_LENGTH = 4;//The length of the metaphone keys produced.  4 is sweet spot
			...
			public DoubleMetaphoneInternal()
			{
			...
			///     of this word.</param>
			public DoubleMetaphoneInternal(String word)
			{
				//Leave room at the end for writing a bit beyond the length; keys are chopped at the end anyway
				m_primaryKey = new StringBuilder(METAPHONE_KEY_LENGTH + 2);
				m_alternateKey = new StringBuilder(METAPHONE_KEY_LENGTH + 2);
			...
			/// <summary>Static wrapper around the class, enables computation of metaphone keys without instantiating a class.</summary>
			...
			///		Ref to var to receive alternate metaphone key, or be set to null if the word has no alternate key by double metaphone
			///	</param>
			static public void doubleMetaphone(String word, ref String primaryKey, ref String alternateKey)
			{
				DoubleMetaphoneInternal mp = new DoubleMetaphoneInternal(word);
